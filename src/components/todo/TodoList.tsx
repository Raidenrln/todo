...
const TodoList = () => {
  const { todos, handleSave, handleOnchange, handleEdit, handleOnchangeDate } = useTodos();
  ...
  return (
    <div className="flex flex-col gap-2 bg-white min-h-0">
      {todos.map((t) => {
        return (
          <div
            key={t.id}
            className="flex flex-col items-center rounded-xl overflow-hidden shadow-sm border border-stone-200"
          >
            ...
            <div className="w-full p-4">
              <div className="w-full flex gap-2 justify-between">
                {t.isEditable ? (
                  <input
                    ...
                    value={t.name}
                    onChange={(e) => handleOnchange(t, e.target.value)}
                  ></input>
                ) : (
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 mt-3">
                <div
                  className={`${STATUS[t.status].chip} inline-block text-xs font-medium rounded-full px-3 py-1 `}
                >
                  <span>{t.status}</span>
                </div>
